...
files = ["PC4", "CM1", "MW1", "PC1", "PC3"]

# CLARANS

dict1 = {"bal1": 0, "auc1": 0, "recall1": 0, "accu1":0}
for index in range(len(files)):
  ...
  for ind in range(20):
    ...
    X_train_clara = np.array(X_train)
    clarans_instance = clarans(X_train_clara.tolist(), k, 6, 4)
    (ticks, result) = timedcall(clarans_instance.process)
    #returns the clusters 
    clusters = clarans_instance.get_clusters();
    train_df = pd.DataFrame(X_train_clara, columns=col[:-2])
    train_df['Defective'] = y_train
    train_df['Cluster'] = [-1]*len(train_df)
    ...
    for i in range(k):
      ...
      D = len(temp_df[temp_df['Defective']==1])
      nd = len(temp_df[temp_df['Defective']==-1])
      r = D/len(temp_df)
      if r > p:
        chosendf = temp_df[temp_df['Defective']==1]
      else:
        chosendf = temp_df[temp_df['Defective']==-1]
    ...
    if n1 != 0:
      pf = n0 / n1
      bal = 1 - (np.sqrt(pf * pf + (1 - recall1) * (1 - recall1)))/ np.sqrt(2)
